...
puts "Il y a #{@handles.length} journalistes dans ce tableau"
...
puts "#{i} handles contiennent un numéro" 


j = 0
for element in handles
	if element =~ /Aude/ || element =~ /aude/
		j = j+1
	end
end
puts "Il y a #{j} Aude dans la liste"
...
puts "#{numbers} handles contiennent des chiffres"


def prenomaude
 	@handles.count {
		|element| element =~ /Aude/ || element =~ /aude/
	}
end

puts "Il y a #{prenomaude} Aude dans la liste"


def startswithcaps
 	@handles.count {
 		|element| element[1] =~ /[A-Z]/
 	}
end

puts "Il y a #{startswithcaps} handles qui commencent par une majuscule"
...
puts "#{containscap} handles contiennent une majuscule"


# Combien de _ dans tous les pseudos confondus ?
def countunderscores
	string = @handles.join
	string.count('_')
end
...
puts "Il y a #{countunderscores} underscores dans la liste"
...
puts " "
puts "appuie sur entrée pour la suite"
gets
...
def sortalpha
	...
	@handles.sort_by{ |element| element.downcase }
end

puts "voici les handles par ordre alphabétique : #{sortalpha}"
